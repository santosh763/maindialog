...
class MakeReservationDialog extends ComponentDialog {
    ...
    async getSecureQuestion(step) {
        ...
        if (step.result) {
            step.values.name = step.result;
            return await step.prompt(TEXT_PROMPT, 'Please answer your security question below \n enter your nickname?');
        }
    }

    async getNumberOfParticipants(step) {
        step.values.secureQuestion = step.result;
    ...
    async summaryStep(step) {
        if (step.result === true) {
            ...
            await step.context.sendActivity('Booking successfully made. Your booking id is : 12345678');
            endDialog = true;
